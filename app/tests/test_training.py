...
def test_asiati_onboarding_template_repairs_existing_draft_without_duplicate(db):
    ...
    module_one = next(
        module
        for module in payload["modules"]
        if module["title"] == "Módulo 1 · Bienvenida a ASIATI"
    )
    assert module_one["lessons"][0]["title"] == "Bienvenida a ASIATI"
    assert module_one["lessons"][0]["video_url"] == service.ASIATI_ONBOARDING_MODULE_1_VIDEO_URL
    module_two = next(
        module
        for module in payload["modules"]
    ...
    assert any(
        ...
        and lesson["video_url"] == service.ASIATI_ONBOARDING_MODULE_2_VIDEO_URL
        for lesson in module_two["lessons"]
    )
    module_three = next(
        module
        for module in payload["modules"]
        if module["title"] == "Módulo 3 · Conoce al equipo"
    )
    assert [lesson["title"] for lesson in module_three["lessons"]] == [
        member[0] for member in service.ASIATI_ONBOARDING_MODULE_3_TEAM
    ]
    module_four = next(
        module
        for module in payload["modules"]
        if module["title"] == service.ASIATI_ONBOARDING_MODULE_4_TITLE
    )
    assert [lesson["title"] for lesson in module_four["lessons"]] == [
        service.ASIATI_ONBOARDING_MODULE_4_VIDEO_TITLE,
        service.ASIATI_ONBOARDING_MODULE_4_CHECKLIST_TITLE,
    ]
    assert module_four["lessons"][0]["video_url"] == service.ASIATI_ONBOARDING_MODULE_4_VIDEO_URL
    assert module_four["lessons"][0]["duration_seconds"] == 105
    assert module_four["lessons"][1]["checklist_items"] == service.ASIATI_ONBOARDING_MODULE_4_CHECKLIST_ITEMS
    module_five = next(
        module
        for module in payload["modules"]
...
def test_asiati_onboarding_template_scaffolds_short_journey(db):
    ...
    assert "Bienvenida a ASIATI" in video_titles
    assert "Módulo 1 · ASIATI" not in video_titles
    assert "Quiénes somos y qué hacemos" in video_titles
    assert "Módulo 2 · ASIATI" not in video_titles
    assert "Módulo 3 · ASIATI" not in video_titles
    assert "Módulos 1–3 · Introducción corporativa" not in video_titles
    assert "Permisos y vacaciones" in video_titles
    ...
    assert "Lo que esperamos de ti" in video_titles
    assert "Módulo 7 · Lo que esperamos de ti" not in video_titles

    welcome_lesson = next(
        lesson
        for module in payload["modules"]
        for lesson in module["lessons"]
        if lesson["title"] == "Bienvenida a ASIATI"
    )
    assert welcome_lesson["video_url"] == service.ASIATI_ONBOARDING_MODULE_1_VIDEO_URL
    assert welcome_lesson["duration_seconds"] == 58
    assert welcome_lesson["estimated_minutes"] == 1
    assert welcome_lesson["duration_known"] is True
    assert welcome_lesson["is_optional"] is False
    ...
    module_two_video = next(
        lesson
        for lesson in module_two["lessons"]
        if lesson["title"] == "Quiénes somos y qué hacemos"
    )
    assert module_two_video["video_url"] == service.ASIATI_ONBOARDING_MODULE_2_VIDEO_URL
    assert module_two_video["duration_seconds"] == 38
    assert module_two_video["estimated_minutes"] == 1
    assert module_two_video["duration_known"] is True
    assert module_two_video["is_optional"] is False
    assert all(
        lesson["is_optional"] is True
        for lesson in module_two["lessons"]
        if lesson["content_type"] in {"ARTICLE", "RESOURCE"}
        and lesson["title"] != "Quiénes somos y qué hacemos"
    )
    ...
    expected_team = service.ASIATI_ONBOARDING_MODULE_3_TEAM
    assert module_three["lesson_count"] == len(expected_team) == 12
    assert module_three["content_item_count"] == 12
    assert [lesson["title"] for lesson in module_three["lessons"]] == [
        member[0] for member in expected_team
    ]
    expected_by_name = {
        name: (url, duration)
        for name, url, duration in expected_team
    }
    for lesson in module_three["lessons"]:
        expected_url, expected_duration = expected_by_name[lesson["title"]]
        assert lesson["video_url"] == expected_url
        assert lesson["duration_seconds"] == expected_duration
        assert lesson["content_type"] == "VIDEO"
        assert lesson["duration_known"] is True
        assert lesson["is_optional"] is False
    ...
    assert module_four["lesson_count"] == 2
    module_four_video = next(
        lesson
        for lesson in module_four["lessons"]
        if lesson["title"] == service.ASIATI_ONBOARDING_MODULE_4_VIDEO_TITLE
    )
    assert module_four_video["video_url"] == service.ASIATI_ONBOARDING_MODULE_4_VIDEO_URL
    assert module_four_video["duration_seconds"] == 105
    assert module_four_video["estimated_minutes"] == 2
    assert module_four_video["duration_known"] is True
    assert module_four_video["is_optional"] is False

    module_four_checklist = next(
        lesson
        for lesson in module_four["lessons"]
        if lesson["title"] == service.ASIATI_ONBOARDING_MODULE_4_CHECKLIST_TITLE
    )
    assert module_four_checklist["content_type"] == "CHECKLIST"
    ...
    module_seven_video = next(
        ...
        if lesson["title"] == service.ASIATI_ONBOARDING_MODULE_7_VIDEO_TITLE
    )
    assert module_seven_video["video_url"] == service.ASIATI_ONBOARDING_MODULE_7_VIDEO_URL
    assert module_seven_video["duration_seconds"] == 55
    assert module_seven_video["estimated_minutes"] == 1
    assert module_seven_video["duration_known"] is True
    assert module_seven_video["is_optional"] is False

    module_seven_ack = next(
        lesson
        for lesson in module_seven["lessons"]
        if lesson["title"] == service.ASIATI_ONBOARDING_MODULE_7_ACK_TITLE
    )
    assert module_seven_ack["content_type"] == "CHECKLIST"
    assert module_seven_ack["checklist_items"] == service.ASIATI_ONBOARDING_MODULE_7_ACK_ITEMS
    assert module_seven_ack["is_optional"] is False
